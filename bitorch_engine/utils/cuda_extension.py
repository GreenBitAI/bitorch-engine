...
def get_kwargs() -> Dict[str, Any]:
    ...
    kwargs = {
        "extra_compile_args": {
            ...
            "nvcc": [
                ...
                f"-arch={os.environ.get('BIE_CUDA_ARCH', 'sm_80')}", # Set CUDA architecture, default to 'sm_80'
                "-DARCH_SM_75" if os.environ.get('BIE_CUDA_ARCH') == 'sm_75' else "-DARCH_SM_80", # Define architecture macro based on BIE_CUDA_ARCH
            ],
        },
    }
    ...
    if major > 11:
        ...
        kwargs["extra_compile_args"]["nvcc"].append("-ccbin=/usr/bin/gcc-11") # Specify GCC 11 as the host compiler for nvcc
    return kwargs
